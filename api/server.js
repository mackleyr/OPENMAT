...
const allowed = new Set(
  [
    "http://localhost:3000",
    "http://127.0.0.1:3000",
    process.env.OPENMAT_DOMAIN,
    "https://openmat.vercel.app",
    "https://openmatapi.vercel.app",
  ].filter(Boolean)
);
app.use(
  cors({
...
app.post("/api/profile/upsert", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "profile_upsert_failed", details: err?.message || "unknown" });
  }
});
...
app.get("/api/profile/get", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "profile_get_failed", details: err?.message || "unknown" });
  }
});
